...
namespace ws {
    ...
    struct Message
    {
        ...
        Message(const std::string &str, FrameType type) : type(type), data(str.c_str()), len((int)str.size()) {}
        Message(std::string &&str, FrameType type) : type(type), data(str.c_str()), len((int)str.size()) {}
        FrameType type;
        const char* data;
