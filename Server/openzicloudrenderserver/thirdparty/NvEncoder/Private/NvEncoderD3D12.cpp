...
void NvEncoderD3D12::EncodeFrame(std::vector<uint8_t> &vPacket, NV_ENC_PIC_PARAMS *pPicParams)
{
    vPacket.clear();
    if (!IsHWEncoderInitialized())
    {
...
void NvEncoderD3D12::EndEncode(std::vector<uint8_t> &vPacket)
{
    vPacket.clear();
    if (!IsHWEncoderInitialized())
    {
...
void NvEncoderD3D12::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_RESOURCE_D3D12*>& vOutputBuffer, std::vector<uint8_t>& vPacket, bool bOutputDelay)
{
...
void NvEncoderD3D12::FlushEncoder()
{
    ...
    try
    {
        std::vector<uint8_t> pOutputBuffer;
