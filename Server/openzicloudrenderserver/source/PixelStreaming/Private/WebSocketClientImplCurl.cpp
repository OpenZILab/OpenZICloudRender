...
void WebSocketClientImplCurl::Connect(const std::string Url)
{
...
{
    Message msg(ws::FrameType::Close, NULL, 0);
    Send(msg);
}
...
#endif
    mask_key.integer = reinterpret_cast<long long>(buff)+1;  // get a random integer
#ifdef _MSC_VER
#pragma warning( pop )
#endif
    if (!buff)
...
size_t WebSocketClientImplCurl::OnMessageReceived(char * ptr, size_t size, size_t nmemb, void * userdata)
{
    ...
    while (remaining)
    {
        ...
        CHECK_REMAINING(payloadlen)
        Message msg(frame_type, tmp, static_cast<int>(payloadlen));
