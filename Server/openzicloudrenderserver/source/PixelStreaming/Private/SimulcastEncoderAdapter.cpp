...
namespace
{
    ...
    // OpenZI::CloudRender::FSimulcastEncoderAdapter, but with the stream index it's registered with as
    // the first parameter to Encoded.
    class AdapterEncodedImageCallback : public webrtc::EncodedImageCallback
    {
    public:
        AdapterEncodedImageCallback(OpenZI::CloudRender::FSimulcastEncoderAdapter *adapter,
                                    size_t stream_idx)
    ...
    private:
        OpenZI::CloudRender::FSimulcastEncoderAdapter *const adapter_;
...
{
    FSimulcastEncoderAdapter::FSimulcastEncoderAdapter(webrtc::VideoEncoderFactory *primary_factory, const webrtc::SdpVideoFormat &format)
        : Initialized(false), PrimaryEncoderFactory(primary_factory), VideoFormat(format), EncodedCompleteCallback(nullptr)
    {
    ...
    int FSimulcastEncoderAdapter::EncodeStream(const webrtc::VideoFrame &InputImage, std::shared_ptr<ITextureSource> LayerFrameSource, size_t StreamIdx, bool bSendKeyFrame)
    {
        // // grab the simulcast frame source, extract the frame source for this layer and wrap that in a new frame buffer
        // rtc::scoped_refptr<FLayerFrameBuffer> LayerFrameBuffer = rtc::scoped_refptr<FLayerFrameBuffer>(LayerFrameSource.get());
    ...
    int FSimulcastEncoderAdapter::Encode(const webrtc::VideoFrame &input_image, const std::vector<webrtc::VideoFrameType> *frame_types)
    {
        ...
        const FFrameBuffer *FrameBuffer = static_cast<FFrameBuffer *>(input_image.video_frame_buffer().get());
        ...
        // check(FrameBuffer->GetFrameBufferType() == FFrameBufferType::Simulcast);
        ...
        for (size_t StreamIdx = 0; StreamIdx < StreamInfos.size(); ++StreamIdx)
        {
            ...
            std::shared_ptr<ITextureSource> LayerFrameSource = SimulcastFrameBuffer->GetLayerFrameSource(StreamInfos.size() == 1 ? SimulcastFrameBuffer->GetNumLayers() - 1 : StreamIdx);
