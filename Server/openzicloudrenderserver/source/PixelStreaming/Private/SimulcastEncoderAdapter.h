    ...
    private:
        struct StreamInfo
        {
            StreamInfo(std::unique_ptr<webrtc::VideoEncoder> InEncoder,
                       ...
                       bool bInKeyFrame,
                       bool bInSendStream)
                : Encoder(std::move(InEncoder)), Callback(std::move(InCallback)), FramerateController(std::move(InFramerateController)), Width(InWidth), Height(InHeight), KeyFrameRequest(bInKeyFrame), bSendStream(bInSendStream)
            {
            }
        ...
        int EncodeStream(const webrtc::VideoFrame &InputImage, std::shared_ptr<ITextureSource> LayerFrameSource, size_t StreamIdx, bool bSendKeyFrame);
    };
} // namespace OpenZI::CloudRender